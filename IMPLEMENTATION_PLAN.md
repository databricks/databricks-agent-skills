...
**`scripts/audit_check.py` reports must-fix total 0.** Every must-fix row is at
zero; the three blocked rows are unchanged at PD-4b 19 / TOK-5 21 / COMPAT-1 7;
no advisory row moved. `skills.py validate` exits 0 and 149 tests pass.

Branch chain, each off the previous so counts stayed comparable:

| Branch | Class | Count |
|---|---|---|
| `ralph/phase1-base` | integration of the two divergent heads | must-fix 227 |
| `ralph/spec-10b-basenames` | SPEC-10b | 64 → 0 |
| `ralph/spec-10a-remainder` | SPEC-10a-intra | 19 → 0 |
| `ralph/pd-4-routing` | PD-4a + `databricks-docs` terminology | 3 → 0 |
| `ralph/new-c-root-refs` | NEW-C | 4 → 0 |
| `ralph/desc-1-triggers` | DESC-1 | 6 → 0 |
| `ralph/pd-1-ceilings` | PD-1 / PD-2 / PD-3 | 3 + 3 + 4 → 0 |
| `ralph/pd-6-toc` | PD-6 | 121 → 0 |

Resident set moved once, on DESC-1: **2,975 → 3,347 stable / 3,199 → 3,571
all-in** (+372 tokens, +12.5%), the cost of giving six skills a trigger.

Still open, all by decision rather than effort: PD-4b, TOK-5 and COMPAT-1 are
blocked on maintainer calls; Phase 2's GC-1/GC-8 authoring task and the
advisory PD-8 / MNT-6 rows are untouched; MNT-1b (the regression guards in
`validators.py`) is deliberately last. See **File as issue** and the
...
| ID | Severity | Count |
|---|---|---|
| PD-6 | must-fix | **0 — DONE** (was 121; 125 at audit) |
...
must-fix total **0** — Phase 1 is closed (rollup/advisory not summed); resident set **3,347 stable /
3,571 all-in** (was 2,975 / 3,199; the DESC-1 rewrite added 372 tokens).

---

## 2. PD-6 — missing reference TOCs — 121 → 0 — DONE

- **Finding:** PD-6 — **DONE**, 121 → 0. **The last must-fix class.**
- **Landed** on branch `ralph/pd-6-toc`, off `ralph/pd-1-ceilings`, in four
  commits by skill: metric-views (3) → the four heaviest (46) → the mid-weight
  eight (47) → the remaining twelve skills (26).
- **Population was 121, not the audit's 125**, and the difference is fully
  accounted for: NEW-C shipped 4 TOCs with its relocated files, and PD-1/PD-2
  added 4 new files over 100 lines and then TOC'd them, netting zero.
- **Generation rules** (reusable; they are what made 121 files mechanical):
  - Mask fenced blocks before parsing. These files are dense with SQL, bash and
    Python; a `# comment` inside a fence otherwise becomes a phantom entry.
  - The H1 is the document title, never an entry. All 121 files open with one.
  - Per-file depth: H2+H3 when the file has ≤12 H2s and ≤45 sections, H2-only
    otherwise, so a TOC never buries the file it indexes.
  - GitHub-style anchor dedup (`-1`, `-2`), computed over **every** heading
    including the H1 so a title/section collision cannot alias.
  - Insert after the H1 and its intro, before the first section heading —
    the house style in `databricks-jobs/references/task-types.md`.
- **Two files the generator could not do mechanically:**
  - `spark-python-data-source/references/implementation-template.md` is one
    115-line fenced template whose only section heading sits at line 129. The
    TOC landed there, **outside D8's 60-line window**, so the file stayed in
    the population and took a *second* TOC on the next pass. Fixed by hand.
    **Lesson: D8's window is a placement constraint, not just a detector.**
  - Two TOC labels duplicated a `(Beta)` marker from their own heading and
    pushed blocked row **TOK-5 21 → 23**. The marker is stripped from the
    *label* only; the anchor still derives from the full heading. A sweep must
    not move a blocked row it is not about.
- **Corpus-wide verification after every batch:** 2,034 in-file anchors across
  213 markdown files, **0 unresolved**.
- **Backpressure:** `audit_check.py --only PD-6` → 0; **must-fix total 0**;
  blocked unchanged at 19 / 21 / 7; `skills.py validate` → 0; 149 tests pass.
...
| 9 | `ralph/pd-6-toc` | 2 · PD-6 (4 commits) | DONE (121 → 0) |
