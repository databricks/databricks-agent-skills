...
metadata:
  version: "0.1.1"
parent: databricks-cli
---

# Databricks Apps Development

**FIRST**: Use the parent `databricks-cli` skill for CLI basics, authentication, and profile selection.
...
|-------|------------------------|
| Scaffolding | Parent `databricks-cli` skill (auth, warehouse discovery); run `databricks apps manifest` and use its plugins/resources to build `databricks apps init` with `--features` and `--set` (see AppKit section below) |
...
- **Authentication**: covered by parent `databricks-cli` skill.
...
Before writing any SQL, use the parent `databricks-cli` skill for data exploration — search `information_schema` by keyword, then batch `discover-schema` for the tables you need. Do NOT skip this step.
...
2. **Scaffold** (DO NOT use `npx`; use the CLI only):
   ...
   - **Discovery**: Use the parent `databricks-cli` skill to resolve IDs (e.g. warehouse: `databricks warehouses list --profile <PROFILE>` or `databricks experimental aitools tools get-default-warehouse --profile <PROFILE>`).
