---
name: databricks-agent-migrate-from-model-serving
...
> **Related skills:** For general Model Serving endpoint management, use `databricks-model-serving`; for migrating other workloads to serverless, use `databricks-serverless-migration`. This skill covers only migrating an MLflow ResponsesAgent from Model Serving to Databricks Apps.
...
If you encounter issues during deployment, refer to the **databricks-agent-deploy** skill for detailed guidance.
...
If `databricks bundle deploy` fails because the app already exists, refer to the **databricks-agent-deploy** skill for instructions on binding an existing app to the bundle.
