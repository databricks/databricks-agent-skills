...
app = FastAPI()
db_manager = LakebaseConnectionManager("my-app", "production", "my_database")
...
6. **Handle scale-to-zero wake-up**: First connection after idle may take a few hundred ms; implement retry logic
7. **Log token refresh events**: Helps debug authentication issues
8. **Private Link (March 2026+)**: New autoscaling projects use regional hostnames that include the cloud region. If using Private Link, an additional front-end Private Link endpoint may be required for projects created after March 9, 2026. Existing provisioned Private Link setups are unaffected.
