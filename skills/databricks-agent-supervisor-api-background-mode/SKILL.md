---
name: databricks-agent-supervisor-api-background-mode
...
2. Follow the **databricks-agent-supervisor-api** skill to set up the Supervisor API with hosted tools and permissions. This skill extends that setup with background mode support.
...
Include your `TOOLS` list from the **databricks-agent-supervisor-api** skill's Step 2 if you have hosted tools.
...
MCP server tools (`uc_connection` or `app`) require a multi-turn approval flow — see the **databricks-agent-supervisor-api** skill for the full explanation and example input.
