...
```bash
databricks experimental aitools tools init-template app --name my-app --description "My app description" --warehouse "my_warehouse_id" --profile my-workspace
```

**Notes:**
- App name must be ≤26 characters (dev- prefix adds 4 chars, max total 30)
- For `--warehouse` you can use the default warehouse from `databricks experimental aitools tools get-default-warehouse --profile my-workspace`
...
```bash
databricks experimental aitools tools init-template job --name my_job --profile my-workspace
databricks experimental aitools tools init-template job --name my_job --catalog my_catalog --profile my-workspace
```
...
```bash
databricks experimental aitools tools init-template pipeline --name my_pipeline --language python --profile my-workspace
databricks experimental aitools tools init-template pipeline --name my_pipeline --language sql --catalog my_catalog --profile my-workspace
```
...
```bash
databricks experimental aitools tools init-template empty --name my_project --profile my-workspace
```
...
2. Run: `databricks experimental aitools tools init-template app --name my-app --description "App purpose" --profile my-workspace`
3. The command creates a project with AGENTS.md/CLAUDE.md for guidance

### Creating a Data Pipeline

1. Ask user to choose language (Python or SQL)
2. For SQL: `databricks experimental aitools tools init-template pipeline --name my_pipeline --language sql --profile my-workspace`
3. For Python: `databricks experimental aitools tools init-template pipeline --name my_pipeline --language python --profile my-workspace`
4. Optionally specify catalog: `--catalog my_catalog`

### Creating a Job

1. Choose a job name (underscores only, no hyphens)
2. Run: `databricks experimental aitools tools init-template job --name my_job --profile my-workspace`
