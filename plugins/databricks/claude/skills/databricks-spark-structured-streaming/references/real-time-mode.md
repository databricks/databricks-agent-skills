...
| Compute type | **Classic compute** (Dedicated or Standard access mode) | Standard supports Python only. Serverless is NOT supported for standalone RTM — only inside SDP-on-RTM (see the `databricks-pipelines` skill's SDP real-time mode reference). |
