...
description: "Create or refine an initial Databricks Genie Agent design from Unity Catalog tables, views, and Metric Views. Works inside Databricks Genie Code (native UI) or from an external agent via the Databricks CLI/MCP. Use when users ask to build, bootstrap, or draft a focused Agent, inspect workspace data context, choose data sources, design structured context, examples, sample questions, Chat or Agent benchmarks, or prepare safe proposed Agent changes without source data mutation."
---

# Create Genie Agent

Create a focused Genie Agent using Databricks-native context: inspect Unity Catalog metadata, profile candidate data sources, design the Agent surfaces, and propose changes for approval — without mutating source data.
...
- **(a) Inside Databricks Genie Code (native UI)** — resolve `@`-assets, browse Unity Catalog, run approved notebook/SQL-editor steps, and review the proposed Agent in the native editor.
...
- Do not create or update a live Genie Agent unless the user explicitly asks and approves the proposed changes in Databricks.
...
1. Gather requirements: target audience, Agent purpose, draft title, 3-5 real business questions, known terminology, KPI definitions, fiscal/calendar conventions, default filters, security caveats, and intended benchmark execution target when benchmarks are requested.
...
7. Design the Genie Agent surfaces in priority order — structured context first, free-text instructions last (see `references/agent-design-guide.md` → Design Priorities). The more governed the surface, the earlier it belongs:
   - space description: set this **first** — it states the Agent's purpose/scope and is required for multi-agent routing (supervisor agents delegate based on it)
   ...
   - descriptions: table/Metric View/column descriptions that clarify business meaning and selection boundaries. **Column-level** descriptions are set via `data_sources.tables[].column_configs[].description` — see the verified schema in [references/agents.md → Exact Field Schemas](../references/agents.md#exact-field-schemas-verified-against-the-genie-api)
   - synonyms and display names: map business terms to fields. **Column-level** synonyms go in `column_configs[].synonyms`
   - format assistance and entity matching / prompt matching: set per column via `column_configs[].enable_format_assistance` / `.enable_entity_matching`. Enable **selectively** — only on useful categorical dimensions and filters users name directly; never blanket-enable on IDs, hashes, free text, lat/long, or raw measures. These two toggles are **space-only** (no Metric View equivalent), so emit them even for a fully-governed Metric View source — see [references/agents.md → column_configs](../references/agents.md#exact-field-schemas-verified-against-the-genie-api)
   - hidden fields: remove noisy technical columns from end-user context via `column_configs[].exclude`
   - **Build `column_configs` during creation** — it is optional in the schema, so an Agent created without it ships with no per-column descriptions, synonyms, format assistance, entity matching, or hidden fields. Add an entry for each column that needs any of these, then attach the array to its table in `data_sources`
   - joins: add standard raw-table relationships only when evidence or user confirmation supports them
   - SQL expressions (snippets): reusable filters, expressions, and measures not already governed by Metric Views. **Always prefix every column with the table/Metric View name** (e.g. ``global_sales_assets_metrics.`Trade Date` ``) — bare columns raise `Table name or alias is required for column`. See [references/agent-design-guide.md → Qualify Columns In SQL](references/agent-design-guide.md#qualify-columns-in-sql)
   - example SQL: representative complex question patterns; instructive, not memorized benchmark answers. Qualify every column with its source name here too (same rule as SQL expressions)
...
8. Review the draft against `references/agent-design-guide.md` before proposing live changes.
9. Present the proposed Agent configuration in the Databricks-native editor or chat output for user review. Apply only after the user approves.
...
| Propose / apply the Agent (Step 9) | native Agent editor, user approves in UI | build `serialized_space` JSON and run `databricks genie create-space` / `update-space` — see parent [databricks-genie-agent SKILL.md](../SKILL.md) for the verified field schema and stringify pattern | `manage_genie(action="create_or_update")` |
| Validate the new Agent | ask questions in the Agent UI | `databricks genie start-conversation` + `get-message` + `get-message-attachment-query-result` | `ask_genie` |
...
- **`databricks-metric-views`** — design rules for AI-ready Metric Views (`genie-agent-integration.md`) and the `MEASURE()` query rules Genie must follow (`query-patterns.md`). Use existing Metric Views as governed sources here; when sources are raw tables for reusable KPIs, recommend creating a Metric View with this skill and confirm with the user first.
- **`databricks-genie`** — the parent orchestration hub for the full Agent lifecycle (create, query, export, import, migrate) and the verified `serialized_space` field schema. This subskill provides the *create* methodology; route there for the end-to-end CLI/MCP command surface.
- **`diagnose-genie-agent`** / **`optimize-genie-agent`** — diagnose and tune the Agent after creation.
