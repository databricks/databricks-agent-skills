...
7. Design the Genie Agent surfaces in priority order — structured context first, free-text instructions last (see `references/agent-design-guide.md` → Design Priorities). The more governed the surface, the earlier it belongs:
   ...
   - **Build `column_configs` during creation** — it is optional in the schema, so a Agent created without it ships with no per-column descriptions, synonyms, format assistance, entity matching, or hidden fields. Add an entry for each column that needs any of these, then attach the array to its table in `data_sources`
...
| Propose / apply the Agent (Step 9) | native Agent editor, user approves in UI | build `serialized_space` JSON and run `databricks genie create-space` / `update-space` — see parent [databricks-genie SKILL.md](../SKILL.md) for the verified field schema and stringify pattern | `manage_genie(action="create_or_update")` |
